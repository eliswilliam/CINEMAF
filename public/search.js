...
(function() {
    ...
    function performSearch() {
        ...
        // Tenter d'abord la recherche TMDB, puis fallback sur local
        searchInTMDB(query)
            .then(tmdbResults => {
                if (tmdbResults && tmdbResults.length > 0) {
                    displayResults(tmdbResults, 'TMDB');
                } else {
                    // Aucun résultat TMDB, chercher en local
                    searchInLocalCatalog(query);
                }
            })
            .catch(error => {
                console.warn('Erreur TMDB, recherche locale:', error);
                // En cas d'erreur TMDB, fallback sur recherche locale
                searchInLocalCatalog(query);
            });
    }
    ...
    async function searchInTMDB(query) {
        try {
            ...
            if (data.success && data.results) {
                // Convertir les résultats TMDB au format de notre interface
    ...
    function displayResults(results, source = 'Local') {
        ...
        let sourceLabel = '';
        if (source === 'TMDB') {
            sourceLabel = ' <span style="color: #00d4ff; font-size: 0.9em;">(🌐 Base Mundial TMDB)</span>';
        } else {
            sourceLabel = ' <span style="color: #ffd700; font-size: 0.9em;">(📁 Catálogo Local)</span>';
        }
        
        if (results.length === 0) {
            countP.innerHTML = '0 resultados encontrados';
        } else if (results.length === 1) {
            countP.innerHTML = '1 resultado encontrado' + sourceLabel;
        } else {
            countP.innerHTML = `${results.length} resultados encontrados` + sourceLabel;
        }
    ...
    function createResultCard(item, index) {
        ...
        card.innerHTML = `
            ...
            <div class="search-result-info">
                ...
                ${isTMDB ? '<p style="font-size: 0.75em; color: #00d4ff; margin-top: 8px;">🌐 Informações do TMDB</p>' : ''}
            </div>
        `;
